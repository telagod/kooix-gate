...
## [0.4.x] — 2026-05-22 至 2026-05-28 — Refactor + Product Gaps Closure (188 patches)

> 188 个 patch 的完整版本说明已折叠归档：
> [docs/archive/changelog/CHANGELOG-0.4.x-patch-log.md](./docs/archive/changelog/CHANGELOG-0.4.x-patch-log.md)
>
> 本段是主线汇总；要查具体 0.4.NNN 版本细节请去 archive 文件。

### Theme

Kooix Gate v0.4.0 → v0.4.187（2026-05-22 → 2026-05-28），围绕 **四轮 product-review 自我批判** + **阶段小版收口** + **插件化收口准备**，把 M3 完整产品形态打磨到 v0.5.0-rc 发版水准。

### 四刀 product-review 主线

| 刀 | 版本段 | patch 数 | 主线产出 |
|---|---|---|---|
| **第一刀** | 0.4.65 → 0.4.101 | 37 | SharedHttpClient（4 fastpath 共享 reqwest pool）/ `gate_chat_*` 4 metric / Anthropic+Bedrock `ChatRequest.extra` 透传 / Usage `cache_creation_input_tokens` + o1/o3 details / ProviderError body 脱敏 / Retry ±25% jitter + `stream_safe()` / PgPool 5 env / WASM `host_log` + `host_record_metric` + cwasm 缓存 / `GET /v1/admin/providers/capabilities`（K1 已删）/「空衍」logo 重设计 |
| **第二刀** | 0.4.102 → 0.4.117 | 16 | followup 自我批判稿 / Retry-After RFC 7231 / Usage audio/prediction tokens / SharedClient LRU per-key eviction / chat metrics handler grep test / metric names mod / admin.rs org_members inline mod / channels form-factories `_lib` / host_get_secret_slot 设计稿 / Grafana 4 panel / DataTable virtualize 设计稿 / SECURITY.md 完整化 |
| **第三刀** | 0.4.121 → 0.4.150 | 30 | admin.rs god file 物理拆 8 patch（4368 → 553 行 -87%）/ channels page 拆分 / DataTable virtualize / `HookContext.secrets` + `host_get_secret_slot` / WasmBlobStore / chat e2e bench / chaos fixture / metric const |
| **第四刀** | 0.4.151 → 0.4.171 | 21 | admin/shared.rs 物理拆 / DataTable virtualize 真接 / playground capability gating（K1 已删）/ chaos test toxiproxy 真启 / WASM auto-mount 业务流 |

### 阶段小版（0.4.176-0.4.181，6 patch · 工作区清零）

CI 暂停（GH Free private repo Actions 额度耗尽，触发改 `workflow_dispatch only`）；build-hygiene runbook（240G `target/` 事故复盘）；admin/users 拆 3 子组件（650 → 597 行）。

### 插件化收口（0.4.182-0.4.187，6 patch · M4 候选准备）

文档漂移收口 / docker compose 端到端跑通 / admin Query typed ID 契约修复 / channels drawer prop + SQL bind_idx + 滚动三连修。

### Key files & boundaries

- `admin.rs` 4368 → 553 行（-87%），物理拆 9 子文件：`mod.rs / channels.rs / groups.rs / sso.rs / users.rs / probe.rs / invitations.rs / pricing.rs / org_members.rs`
- `router.rs` 4519 → router/8 子模块；`custom_provider.rs` 3878 → custom_provider/8 子模块；`plugin_manifest.rs` 2193 → plugin_manifest/6 子模块
- 前端：channels +page 1864 → 1199 / admin/groups 1083 → 972 / admin/users 752 → 597 等
- WASM ABI v0：[ADR-0003](./docs/architecture/decisions/ADR-0003-wasm-plugin-abi-v0.md)（Superseded by [ADR-0006](./docs/architecture/decisions/ADR-0006-wasm-abi-v1-component-model.md)）

### 砍单收口（v0.5.0 Wave 1/2 已开始）

0.4.x 系列流水账叙事在 v0.5.0 切换为号池中台叙事。详见：

- [ROADMAP.md](./ROADMAP.md) — M5/M6/M7 三里程碑
- [docs/archive/roadmap/ROADMAP-pre-0.5.0.md](./docs/archive/roadmap/ROADMAP-pre-0.5.0.md) — 旧 ROADMAP 完整快照
- [docs/archive/2026-05-product-reviews/](./docs/archive/2026-05-product-reviews/) — product-review 三件套
