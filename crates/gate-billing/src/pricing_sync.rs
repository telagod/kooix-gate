...
pub async fn sync_from_litellm(
    pool: &PgPool,
    ...
    for (raw_key, val) in &data {
        ...
        let _provider = entry.litellm_provider.as_deref().unwrap_or("");
        ...
        if let Some(cw) = entry.cache_creation_input_token_cost {
            rules.push(RuleInsert {
                model: model.clone(),
                dimension: "cache_write_tokens".into(),
                unit: "per_million_tokens".into(),
                rate: cw * 1_000_000.0,
                conditions: serde_json::json!({}),
                priority: 0,
        ...
        if let Some(i200) = entry.input_cost_per_token_above_200k_tokens {
            rules.push(RuleInsert {
                ...
                dimension: "input_tokens".into(),
                unit: "per_million_tokens".into(),
                rate: i200 * 1_000_000.0,
                conditions: serde_json::json!({"context_above": 200000}),
                priority: 1,
        ...
        if let Some(rt) = entry.output_cost_per_reasoning_token {
            rules.push(RuleInsert {
                model: model.clone(),
                dimension: "reasoning_tokens".into(),
                unit: "per_million_tokens".into(),
                rate: rt * 1_000_000.0,
                conditions: serde_json::json!({}),
                priority: 0,
        ...
        if let Some(at) = entry.input_cost_per_audio_token {
            rules.push(RuleInsert {
                model: model.clone(),
                dimension: "audio_input_tokens".into(),
                unit: "per_million_tokens".into(),
                rate: at * 1_000_000.0,
                conditions: serde_json::json!({}),
                priority: 0,
...
pub async fn sync_from_litellm_upsert(
    ...
    sqlx::query(
        "DELETE FROM pricing_rules WHERE channel_id IS NULL AND description LIKE 'litellm/%'",
    )
    .execute(&mut *tx)
    .await?;
