...
pub fn permissions_of_org_role(role: OrgRole) -> HashSet<Permission> {
    ...
    match role {
        ...
        OrgRole::BillingViewer => {
            HashSet::from([OrgRead, OrgBillingRead, ProjectRead, UsageRead, QuotaRead])
        }
        OrgRole::Member => HashSet::from([OrgRead, ProjectRead]),
    }
}
...
pub fn permissions_of_project_role(role: ProjectRole) -> HashSet<Permission> {
    ...
    match role {
        ...
        ProjectRole::Viewer => HashSet::from([ProjectRead, ApiKeyRead, UsageRead, QuotaRead]),
    }
}
...
pub fn permissions_of_platform_role(role: PlatformRole) -> HashSet<Permission> {
    ...
    match role {
        ...
        PlatformRole::Support => HashSet::from([ChannelRead, GroupRead, AuditRead]),
    }
}
