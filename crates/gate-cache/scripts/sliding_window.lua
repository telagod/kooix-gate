...
-- ARGV[5] = amount (i64, optional, default 1) — 本次请求消耗的单位数
--
-- 返回：
...
--   2. 汇总窗口内 member 的 amount 前缀（旧 member 无前缀按 1）
--   3. 若 count + amount <= limit：ZADD now/"amount:req_id"；返回 allowed=1
...
-- 2. 当前数（兼容旧 member：没有 amount 前缀时按 1）
local count = 0
local members = redis.call('ZRANGE', key, 0, -1)
for _, member in ipairs(members) do
    local sep = string.find(member, ':', 1, true)
    if sep then
        count = count + (tonumber(string.sub(member, 1, sep - 1)) or 1)
    else
        count = count + 1
    end
end
...
if count + amount <= limit then
    -- 3. 允许：写入新 entry
    redis.call('ZADD', key, now, tostring(amount) .. ':' .. req_id)
    redis.call('PEXPIRE', key, window)
    return { 1, count + amount, limit - count - amount, 0 }
else
