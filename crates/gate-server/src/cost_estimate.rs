...
//! - completion_tokens ≈ max_tokens 请求字段（默认 4096）
...
/// 预估上限：100_000 micros = $0.10。避免大请求过度预扣。
pub const MAX_ESTIMATE_MICROS: i64 = 100_000;
...
mod tests {
    ...
    fn short_message_default_max_tokens() {
        // 12 chars / 4 = 3 prompt tokens + 4096 completion = 4099 tokens × 3 = 12297
        let req = make_req(&["Hello world!"], None);
        assert_eq!(estimate_cost_micros(&req, DEFAULT_RATE_PER_TOKEN_MICROS), 12_297);
    }
    ...
    fn caps_at_max_estimate() {
        ...
        // (50000 + 50000) × 3 = 300_000 → capped at 100_000
