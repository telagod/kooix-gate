<script lang="ts">
	...
	import CanaryComparePanel from './_components/CanaryComparePanel.svelte';
	import BindingTable from './_components/BindingTable.svelte';
	import {
		STRATEGIES,
		PROVIDER_COLOR,
		strategyMeta,
	...
	// metricDelta / weightedBaseline / formatMaybeMs / formatMaybeMicros / formatSignedPercentDelta / formatSignedNumberDelta
	// 已抽到 ./_lib/helpers.ts（0.4.63）
...
>
	...
	{#if selectedId && detail}
		...
		<div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 divide-y divide-zinc-200 dark:divide-zinc-700">
			...
			<!-- ═══ Canary Stats ═══ -->
			<CanaryComparePanel {canary} {canaryOnly} {baseline} {stats} />
			...
			<BindingTable
				...
				onUpdatePriority={(v) => (editBindingPriority = v)}
				onUpdateWeight={(v) => (editBindingWeight = v)}
				onUpdateCanaryPercent={(v) => (editBindingCanaryPercent = v)}
			/>
		</div>
	{/if}
</PageShell>
