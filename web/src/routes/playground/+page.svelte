<script lang="ts">
	...
	import {
		...
		MessageSquare, X, Clock, Coins, Zap, ImagePlus, Volume2, Mic
	} from 'lucide-svelte';
	...
	let modelOptions = $derived(
		availableModels.length > 0 ? availableModels.map(m => m.id) : FALLBACK_MODELS
	);
	...
	async function send() {
		...
		if (active.messages.filter(m => m.role === 'user').length === 1) {
			active.title = userMsg.content.slice(0, 30) + (userMsg.content.length > 30 ? '...' : '');
		}
		...
		// remove the empty assistant msg from the messages sent to API
		const apiMsgs = allMsgs.slice(0, -1);
		...
		abortCtrl = chatCompletionStream(
			...
			(chunk) => {
				...
				active.messages = [...active.messages.slice(0, -1), { ...last, content: last.content + chunk }];
				scrollBottom();
			},
			(meta) => {
				if (!active) return;
				const last = active.messages[active.messages.length - 1];
				active.messages = [...active.messages.slice(0, -1), { ...last, meta, finishedAt: Date.now() }];
				streaming = false;
				saveConversations();
			},
	...
	function regenerate() {
		...
		active.messages = msgs.slice(0, -1);
		// re-add empty assistant
		const assistantMsg: Msg = { id: uid(), role: 'assistant', content: '', startedAt: Date.now() };
		...
		const apiMsgs = buildMessages().slice(0, -1);
		const params: ChatParams = {
			model: active.model,
			messages: apiMsgs,
			temperature: active.temperature,
	...
	function applyEdit(msgId: string) {
		if (!active) return;
		const idx = active.messages.findIndex(m => m.id === msgId);
		if (idx === -1) return;
		// update the message and truncate everything after
		const updated = { ...active.messages[idx], content: editContent };
		active.messages = [...active.messages.slice(0, idx), updated];
		active.updatedAt = Date.now();
		editingMsgId = null;
		editContent = '';
		saveConversations();
	...
	function deleteMessage(msgId: string) {
		if (!active) return;
		active.messages = active.messages.filter(m => m.id !== msgId);
		active.updatedAt = Date.now();
		saveConversations();
	}

	function copyContent(msg: Msg) {
	...
	function formatLatency(msg: Msg): string {
		if (!msg.startedAt || !msg.finishedAt) return '';
		const ms = msg.finishedAt - msg.startedAt;
			...
			<div class="flex-1 overflow-y-auto">
				{#each conversations as conv}
					<div
						...
						class={clsx(
							'w-full text-left px-3 py-2.5 text-sm transition-colors group flex items-center justify-between cursor-pointer',
							conv.id === activeId
	...
	<div class="flex-1 flex flex-col min-w-0">
		...
		<div class="flex flex-1 overflow-hidden">
			...
			<div class="flex-1 flex flex-col min-w-0">
				<div bind:this={chatEl} class="flex-1 overflow-y-auto px-4 py-4">
					{#if active && active.messages.length === 0}
						<div class="flex flex-col items-center justify-center h-full text-zinc-400 dark:text-zinc-500">
							...
							<p class="text-xs mt-1.5 text-zinc-400 dark:text-zinc-500">Shift+Enter 换行 · Enter 发送</p>
						</div>
					{:else if active}
										...
										<div class="flex flex-col items-end max-w-[85%]">
											{#if editingMsgId === msg.id}
												<div class="w-full min-w-[300px]">
													<textarea
														...
														rows={3}
														class="w-full rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400 resize-none"
													></textarea>
											...
											{:else}
												...
												<div class="flex gap-0.5 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
													...
													<button onclick={() => copyContent(msg)} class="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800" title="复制">
														{#if copiedId === msg.id}
													...
													<button onclick={() => deleteMessage(msg.id)} class="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800" title="删除">
														<Trash2 size={12} class="text-zinc-400" />
													</button>
												</div>
											{/if}
										</div>
										<div class="w-7 h-7 rounded-full bg-zinc-900 dark:bg-zinc-200 flex items-center justify-center shrink-0 mt-1">
											<UserIcon size={13} class="text-white dark:text-zinc-800" />
										</div>
									</div>
								{:else if msg.role === 'assistant'}
									<!-- Assistant message -->
									<div class="flex gap-3 group py-2">
										<div class="w-7 h-7 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0 mt-1 ring-1 ring-zinc-200 dark:ring-zinc-700">
											<Bot size={13} class="text-zinc-500" />
										</div>
										<div class="flex flex-col max-w-[85%] min-w-0">
											<div class="text-sm text-zinc-900 dark:text-zinc-100">
												{#if msg.content}
													<MarkdownRenderer content={msg.content} streaming={streaming && i === active.messages.length - 1} />
												{:else if streaming && i === active.messages.length - 1}
													<span class="inline-block w-2 h-4 bg-zinc-400 dark:bg-zinc-500 animate-pulse rounded-sm"></span>
												{/if}
											</div>
											<!-- Meta + actions -->
											<div class="flex items-center gap-3 mt-1.5 text-[11px] text-zinc-400 dark:text-zinc-500">
												{#if msg.meta?.usage}
													<span class="inline-flex items-center gap-0.5" title="Token 用量">
														<Zap size={10} />
														{msg.meta.usage.prompt_tokens} / {msg.meta.usage.completion_tokens}
													</span>
												{/if}
												{#if msg.finishedAt && msg.startedAt}
													<span class="inline-flex items-center gap-0.5" title="耗时">
														<Clock size={10} />
														{formatLatency(msg)}
													</span>
												{/if}
												{#if msg.meta?.model}
													<span class="truncate max-w-[150px]" title={msg.meta.model}>{msg.meta.model}</span>
												{/if}
												<div class="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
													...
													{#if i === active.messages.length - 1 && !streaming}
														<button onclick={regenerate} class="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800" title="重新生成">
				...
				{#if error}
					<div class="mx-4 mb-2 px-3 py-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-xs flex items-center gap-2">
						<X size={12} />
						{error}
					</div>
				{/if}
				...
				>
					<div class="max-w-3xl mx-auto">
						...
						<div class="flex gap-2 items-end">
							...
							></textarea>
							{#if streaming}
								<Button variant="destructive" size="sm" onclick={stop} class="rounded-xl h-10 w-10 p-0">
									<Square size={14} />
								</Button>
							{:else}
			...
			{#if showSettings && active}
				<div class="w-72 shrink-0 border-l border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 overflow-y-auto">
					<div class="p-4 space-y-5">
						<div>
							...
							<div class="mb-4">
								...
								<textarea
									bind:value={active.systemPrompt}
									onchange={() => { if (active) { active.updatedAt = Date.now(); saveConversations(); }}}
									placeholder="你是一个有用的助手..."
									rows={4}
									class="w-full rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-400 resize-none"
								></textarea>
							</div>
							...
							<div class="mb-4">
								<div class="flex justify-between text-xs mb-1.5">
									...
									<span class="font-mono text-zinc-700 dark:text-zinc-300">{active.temperature.toFixed(1)}</span>
								</div>
								<input
									type="range"
									min="0" max="2" step="0.1"
									bind:value={active.temperature}
									onchange={() => { if (active) { active.updatedAt = Date.now(); saveConversations(); }}}
							...
							<div class="mb-4">
								<div class="flex justify-between text-xs mb-1.5">
									...
									<span class="font-mono text-zinc-700 dark:text-zinc-300">{active.topP.toFixed(1)}</span>
								</div>
								<input
									type="range"
									min="0" max="1" step="0.05"
									bind:value={active.topP}
									onchange={() => { if (active) { active.updatedAt = Date.now(); saveConversations(); }}}
									class="w-full accent-zinc-900 dark:accent-zinc-100"
								/>
							</div>
							...
							<div class="mb-4">
								<div class="flex justify-between text-xs mb-1.5">
									...
									<span class="font-mono text-zinc-700 dark:text-zinc-300">{active.maxTokens}</span>
								</div>
								<input
									type="number"
									min="1" max="128000" step="256"
									bind:value={active.maxTokens}
									onchange={() => { if (active) { active.updatedAt = Date.now(); saveConversations(); }}}
							...
							<div>
								...
								<div class="space-y-2 text-xs">
									<div class="flex justify-between">
										<span class="text-zinc-400">消息数</span>
										<span class="text-zinc-700 dark:text-zinc-300">{active.messages.length}</span>
									</div>
									<div class="flex justify-between">
										<span class="text-zinc-400">创建时间</span>
										<span class="text-zinc-700 dark:text-zinc-300">{formatDate(active.createdAt)}</span>
									</div>
									{#if active.messages.filter(m => m.meta?.usage).length > 0}
										...
										<div class="flex justify-between">
											<span class="text-zinc-400">总 Tokens</span>
											<span class="font-mono text-zinc-700 dark:text-zinc-300">{totalIn + totalOut}</span>
										</div>
										<div class="flex justify-between">
											<span class="text-zinc-400">输入 / 输出</span>
											<span class="font-mono text-zinc-700 dark:text-zinc-300">{totalIn} / {totalOut}</span>
										</div>
									{/if}
								</div>
