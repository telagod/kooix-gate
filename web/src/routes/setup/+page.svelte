	...
	<Card class="w-full max-w-lg p-8">
		{#if done}
			<div class="text-center">
				...
				<h1 class="text-2xl font-bold text-zinc-900 mb-2">初始化完成</h1>
				<p class="text-sm text-zinc-500 mb-6">管理员账号 <span class="font-mono font-medium text-zinc-700">{resultEmail}</span> 已创建</p>
				<Button class="w-full" onclick={handleLogin} disabled={loading}>
					{loading ? '登录中...' : '进入控制台'}
				</Button>
				{#if error}
					<p class="text-sm text-red-600 bg-red-50 rounded-md px-3 py-2 mt-4">{error}</p>
				{/if}
			</div>
		{:else}
			<div class="mb-6">
				<h1 class="text-2xl font-bold text-zinc-900">Kooix Gate 初始化</h1>
				<p class="mt-1 text-sm text-zinc-500">首次使用，请创建管理员账号和默认组织</p>
				<div class="flex gap-2 mt-4">
					<div class="flex-1 h-1 rounded-full {step >= 1 ? 'bg-zinc-900' : 'bg-zinc-200'}"></div>
					<div class="flex-1 h-1 rounded-full {step >= 2 ? 'bg-zinc-900' : 'bg-zinc-200'}"></div>
				</div>
				<p class="text-xs text-zinc-400 mt-2">步骤 {step} / 2</p>
			</div>

			<form onsubmit={handleSubmit} class="space-y-4">
				{#if step === 1}
					<div>
						<label for="email" class="block text-sm font-medium text-zinc-700 mb-1">管理员邮箱</label>
						<Input id="email" type="email" placeholder="admin@example.com" bind:value={email} disabled={loading} autocomplete="email" />
					</div>
					<div>
						<label for="password" class="block text-sm font-medium text-zinc-700 mb-1">密码（至少 8 位）</label>
						<Input id="password" type="password" placeholder="••••••••" bind:value={password} disabled={loading} autocomplete="new-password" />
					</div>
					<div>
						<label for="passwordConfirm" class="block text-sm font-medium text-zinc-700 mb-1">确认密码</label>
				...
				{:else}
					<div class="grid grid-cols-2 gap-3">
						<div>
							<label for="orgName" class="block text-sm font-medium text-zinc-700 mb-1">组织名称</label>
							<Input id="orgName" placeholder="My Org" bind:value={orgName} disabled={loading} />
						</div>
						<div>
							<label for="orgSlug" class="block text-sm font-medium text-zinc-700 mb-1">组织 Slug</label>
							<Input id="orgSlug" placeholder="my-org" bind:value={orgSlug} disabled={loading} />
						</div>
					</div>
					<div class="grid grid-cols-2 gap-3">
						<div>
							<label for="projName" class="block text-sm font-medium text-zinc-700 mb-1">项目名称</label>
							<Input id="projName" placeholder="My Project" bind:value={projectName} disabled={loading} />
						</div>
						<div>
							<label for="projSlug" class="block text-sm font-medium text-zinc-700 mb-1">项目 Slug</label>
							<Input id="projSlug" placeholder="my-project" bind:value={projectSlug} disabled={loading} />
						</div>
					</div>
					<div class="bg-zinc-50 border border-zinc-200 rounded-lg px-4 py-3">
						<p class="text-xs text-zinc-500">组织和项目可在初始化后随时修改，这里用默认值即可快速开始。</p>
					</div>
				{/if}
