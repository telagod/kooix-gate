...
- `PageShell` / `StatePanel` / `DataToolbar` / `FilterPanel` / `DataTable` 已作为页面与数据页模板落地，并迁移 `admin/audit`、`admin/users`、`admin/incidents`、`admin/pricing`、`admin/requests`、`admin/sso`、`orgs/[orgId]/quotas`、`orgs/[orgId]/billing`、`orgs/[orgId]/projects`、`orgs/[orgId]/projects/[projectId]`、`orgs/[orgId]/projects/[projectId]/keys`、`usage/requests`、`channels/+page`、`channels/[channelId]` 代表页；后续新增数据页继续优先复用模板。
