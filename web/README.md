...
UI 文案中文主干 + 英文术语的约束由 `src/tests/ui-copy.test.ts` 锁定；新增控制台页面时应保留 Provider / Channel / API Key / SSO / OIDC / Redis / PG 等术语，但不要回退成纯英文标签。
数据页状态优先复用 `src/lib/table-state.ts`：page size / offset、`sort_by` / `sort_dir`、column visibility、saved filters 都在该 helper 做规范化与 localStorage 持久化，页面只负责把状态映射到 API query。当前 `/setup` 已验证首次初始化 `AuthFrame`、theme toggle 与两步 bootstrap 模板路径；`/admin/audit` 已验证服务端 pagination / sort，`/admin/users` 已验证 page size、列显隐与筛选持久化推广路径，`/admin/channels` 已验证渠道仪表盘 `PageShell`、StatePanel 与最近错误 `DataTable` 模板路径，`/admin/groups` 已验证渠道分组 `PageShell`、Canary 对比表与 binding 表格模板路径，`/admin/incidents` 已清掉 native table 缺口，`/admin/sso` 已验证 SSO Provider 搜索工具栏、清除搜索与 provider/enabled badges 模板路径，`/usage` 已验证用量仪表盘 PageShell、range/group toolbar 与 StatePanel 状态模板路径，`/orgs/[orgId]/quotas` 已验证 `DataToolbar` 筛选 + `DataTable` 分组表格路径，`/orgs/[orgId]/billing` 已验证月账单工具栏、导出摘要与两组分解表迁移路径，`/orgs/[orgId]/projects` 已验证 Project 列表表格、Org invite 与创建表单模板路径，`/orgs/[orgId]/projects/[projectId]` 已验证 Project 设置、Project invite、API Key quick create 与模型别名模板路径，`/orgs/[orgId]/projects/[projectId]/keys` 已验证 API Key 表格、一次性明文展示与撤销确认模板路径。
...
- 预设清单在 `src/lib/plugin-presets.ts`，当前 UI 覆盖 OpenAI-compatible、vLLM、LM Studio、Ollama OpenAI endpoint、LocalAI、Xinference、Anthropic Messages、Azure OpenAI、Vertex AI OpenAI、Gemini、DeepSeek、Mistral、Cohere、Ollama、Groq、Together、OpenRouter、Moonshot、智谱、通义千问、零一万物、Bedrock Converse；后端 manifest 也接受 `openai` alias。
