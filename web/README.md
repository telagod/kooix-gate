...
数据页状态优先复用 `src/lib/table-state.ts`：page size / offset、`sort_by` / `sort_dir`、column visibility、saved filters 都在该 helper 做规范化与 localStorage 持久化，页面只负责把状态映射到 API query。当前 `/admin/audit` 已验证服务端 pagination / sort，`/admin/users` 已验证 page size、列显隐与筛选持久化推广路径，`/admin/incidents` 已清掉 native table 缺口。
