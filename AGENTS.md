...
- 基础设计语言集中在 `web/src/lib/design/classes.ts`，页面禁止复制基础按钮/输入框/卡片长 class
- 可复用页面模板放 `web/src/lib/components/templates/`：`PageShell` / `AuthFrame` / `SectionCard` / `StatePanel` / `ModalFrame` / `DataToolbar` / `FilterPanel` / `DataTable`
- 组件放 `web/src/lib/components/ui/`，页面级组件直接放路由
- 使用 `cn`（clsx + tailwind-merge，来自 `$lib/design`）做条件 class 拼接；底层 token 可用 `text` / `surface` / `buttonClass` / `controlClass`
- 字体：Inter（sans）+ JetBrains Mono（mono），在 `app.css @theme` 配置
- 模板说明见 `web/src/lib/design/README.md`
- `npm run check` 是前端门禁，需保持 `0 errors / 0 warnings`
